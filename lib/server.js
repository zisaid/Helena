...
server.all = function (path, interseptorFunction) {
    let paths = formatPath(path);
    let savePath = '^\\/' + paths.join('\\/') + '$';
    savePath = new RegExp(savePath.replace('*', '(.*?)'));
    interseptorList.push({path: savePath, interseptorFunction: interseptorFunction});
};
...
function doInterseptor(req, res) {
    interseptorList.forEach(value => {
        if(value.path.test(req.router)){
            value.interseptorFunction(req, res);
        }
    });
}
